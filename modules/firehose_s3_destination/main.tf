...
module "errors_log" {
  ...
  name = "${var.delivery_stream_name}Errors"
  retention_days = 7
}

resource "aws_cloudwatch_log_stream" "errors" {
  log_group_name = module.errors_log.log_group.name
  name = "errors"
}
...
resource "aws_kinesis_firehose_delivery_stream" "stream" {
  ...
  s3_configuration {
    ...
    cloudwatch_logging_options {
      enabled = true
      log_group_name = module.errors_log.log_group.name
      log_stream_name = aws_cloudwatch_log_stream.errors.name
    }
  }
    ...
    [
        ...
        },
        {
           ...
           "Resource": [
               ...
               "${module.errors_log.log_group.arn}:*"
           ]
        }
    ]
}
